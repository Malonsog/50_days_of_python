...
def sorted_names(list_of_names: list[str]) -> list[str]:
    last_name_first = []
    for name in list_of_names:
        last_name_first.append(name.split(" ")[1] + ", " + name.split(" ")[0])
    return sorted(last_name_first)


test_list = ["Beyonce Knowles", "Alicia Keys", "Katie Perry", "Chris Brown", "Tom Cruise"]
print(sorted_names(test_list))
